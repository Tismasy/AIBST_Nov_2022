...
```{r}
b <- head(seurat_object@meta.data)
seurat_object@assays
head(seurat_object@assays, n=30)
```
...
```{r}
lyz <- FetchData(object = seurat_object, vars = "LYZ" )
lyz <- FetchData(object=seurat_object, vars = c("nCount_RNA", "LYZ"))
head(lyz)
```
...
#To find number of genes with length of 300
length(WhichCells(seurat_object, expression = LYZ > 300))
```
...
```{r}
violin <- VlnPlot(object = seurat_object,
...
VlnPlot(seurat_object, features = "nFeatures_RNA", "nCount_RNA", "percent_mt")

VlnPlot(seurat_object, features = "nFeatures_RNA", "nCount_RNA")
```
...
FeatureScatter(seurat_object,
               feature1 = "nCount_RNA", feature2 = "percent_mt")
```
...
```{r}
ggplot_lyz_raw <- ggplot(FetchData(seurat_object, vars= c("LYZ"), slot = "counts")
, aes(LYZ)) +
    geom_histogram(bins = 100) +
    coord_cartesian(ylim = c(0, 500)) +
    cowplot::theme_cowplot()
ggplot_lyz_normalised <- ggplot(FetchData(seurat_after_qc, vars= c("LYZ"), slot = "data")
, aes(LYZ)) +
    geom_histogram(bins = 100) +
    coord_cartesian(ylim = c(0, 500)) +
    cowplot::theme_cowplot()
...
```{r}
seurat_after_qc@assays$RNA@var.features
...
```{r}
seurat_markers_all <- FindAllMarkers(seurat_after_qc, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25
