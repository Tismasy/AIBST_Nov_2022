...
```{r}
iris_raw %>%
  group_by(species) %>% summarise(across(where(is.numeric), mean))
...
iris_raw %>% 
  group_by(species) %>% summarise(across(where(is.numeric), mean)) %>% filter(sepal_length<6)
...
ggplot(exprs_assay_long, aes(x=value)) +
  geom_histogram(fill = "grey", bins = 50, aes(color=gene))+
  facet_wrap(~gene)
