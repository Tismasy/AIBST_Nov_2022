...
abline(v = median(z), col = "red", lty = "dashed")
abline(v = mean(z)-2*sd(z), col = "blue")
abline(v = mean(z)+2*sd(z), col = "blue")
abline(v = median(z)-mad(z), col = "blue", lty = 2)
abline(v = median(z)+mad(z), col = "blue", lty = 2)
```
...
```{r}
set.seed(1)
y <- rnorm(n = 1000000, mean = 10, sd = 5)
hist(y, breaks = 1000)
...
```{r}
groupA <- df$gene_gene_exprs[df$group=="groupA"]
groupB <- df$gene_gene_exprs[df$group=="groupB"]
t.test(groupA, groupB)
```

**What is the p-value?**
```{r}
t_test <- t.test(groupA, groupB)
```
...
```{r}
#changing from dataframe to matrix
...
name_uninfected <- cell_info$sample[cell_info$Infection !="STM-LT2"]
gene_1_uninfected <- matrix[index, names_infected]
...
```{r}
head(t.test_pvalues, n=10)
hist(t_test_pvalues)
hist(t_test_pvalues, breaks = 25)
```
...
```{r}
result_bh <- p.adjust(t_test_pvalues, method = "bonferroni")
