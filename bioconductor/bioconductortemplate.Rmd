...
```{r}
Person <- function(name = "Tsitsi", age = 21) {
...
```{r}
truseq_adapters <- Biostrings::readDNAStringSet("TruSeq3-PE-2.fa")
truseq_adapters
```
...
```{r}
sample_metadata <- read.csv("gene_metadata.csv", row = 1)
head(sample_metadata)
```

```{r}
gene_metadata <- read.csv("gene_metadata.csv", row = 1)
head(gene_metadata)
```
